...
PPL 后端算子的实现位于 ``tpu-mlir/lib/PplBackend/src`` 目录；如果是发布包，则在 TPU-MLIR 发布包的 ``PplBackend/src`` 目录。有关如何编写 PPL 源码的详细信息，请参考 ``tpu-mlir/cross_toolchains/ppl_compile/doc`` 中的文档。
...
1. PPL 编译器存储在 ``cross_toolchains/ppl_compile`` 目录中，由 ``download_toolchains.sh`` 按需下载。更新 PPL 版本时，修改 ``download_toolchains.sh`` 中的 ``PPL_VERSION`` 和 ``PPL_URL`` 变量后重新运行即可。
